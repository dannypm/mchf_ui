...
const GUI_BITMAP * settings1[] = {
  &bmicon_compref,   &bmicon_compref,   &bmicon_compref,   &bmicon_compref, &bmicon_compref
};
  
static void Startup(WM_HWIN hWin, uint16_t xpos, uint16_t ypos);

K_ModuleItem_Typedef  settings1_board =
{
  4,
  "Standard DSP Menu",
  settings1,
...
static void _cbDialog(WM_MESSAGE * pMsg)
{
	...
	switch (pMsg->MsgId)
	{
		case WM_INIT_DIALOG:
		{
			...
			hItem = WM_GetDialogItem(pMsg->hWin, ID_SPINBOX_0);
			SPINBOX_SetFont(hItem, &GUI_FontAvantGarde32);
			SPINBOX_SetTextColor(hItem,SPINBOX_CI_ENABLED,GUI_STCOLOR_LIGHTBLUE);
